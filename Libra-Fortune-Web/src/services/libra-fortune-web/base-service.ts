...
abstract class BaseService<T extends API.BaseDTO, Q extends API.PaginationQuery> {
  ...
  async get(id: string): Promise<API.ApiResponse<T>> {
    return request<API.ApiResponse<T>>(`http://127.0.0.1:7041/api${this.url}/${id}`, {
      method: 'get',
    });
  }
  ...
  async list(params?: Q): Promise<API.ApiResponse<T[]>> {
    ...
    return request<API.ApiResponse<T[]>>(`http://127.0.0.1:7041/api${this.url}`, {
      method: 'get',
      params,
    });
  }
  ...
  async listByPage(params: API.PaginationQuery): Promise<API.ProTableData<T>> {
    this.camelToUnderscore(params);
    return request<API.ApiResponse<API.PaginationResponse<T>>>(
      `http://127.0.0.1:7041/api${this.url}`,
      {
        method: 'get',
        params,
      },
  ...
  async update(record: T): Promise<API.ApiResponse<T>> {
    return request<API.ApiResponse<T>>(`http://127.0.0.1:7041/api${this.url}/${record.id}`, {
      method: 'put',
      data: record,
    });
  }
  ...
  async remove(id: string): Promise<API.ApiResponse<string>> {
    return request<API.ApiResponse<string>>(`http://127.0.0.1:7041/api${this.url}/${id}`, {
      method: 'delete',
    });
  }
}
